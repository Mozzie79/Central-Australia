...
// CONTRACTOR_ALLOCATIONS tags this product 'mySQL' (lowercase m); no other config
// (Assets/Payroll/HR/BSO/CSO) has any entry for this product, so casing is moot there.
const ASSET_PRODUCT = 'MYSQL';
const PRODUCT = 'mySQL';

export function runMrDbHostingMysqlValidation(): SheetValidation {
  const fixture = loadFixture('mrDbHostingMysql.json');
